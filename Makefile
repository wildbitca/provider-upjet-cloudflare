...
export TERRAFORM_PROVIDER_VERSION ?= 5.18.0
...
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-cloudflare_v5.18.0
...
# Override with specific groups to build a subset: make build.family FAMILY_SUBPACKAGES="config dns zone"
FAMILY_SUBPACKAGES ?= $(FAMILY_ALL_GROUPS)

# Map subpackage name to xpkg repository name
define map_repo_name
$(if $(filter config,$(1)),provider-family-cloudflare,$(if $(filter monolith,$(1)),provider-upjet-cloudflare,provider-cloudflare-$(1)))
endef
...
build.family.image:
	...
	@cp cluster/images/provider-upjet-cloudflare/Dockerfile $(IMAGE_TEMP_DIR) || $(FAIL)
	@cp cluster/images/provider-upjet-cloudflare/terraformrc.hcl $(IMAGE_TEMP_DIR) || $(FAIL)
