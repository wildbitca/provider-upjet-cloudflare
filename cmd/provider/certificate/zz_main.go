...
func main() {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-cloudflare-certificate",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_certificate(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetCloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_certificate(mgr, namespacedOpts), "Cannot setup namespaced UpjetCloudflare controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_certificate(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetCloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_certificate(mgr, namespacedOpts), "Cannot setup namespaced UpjetCloudflare controllers")
	}
