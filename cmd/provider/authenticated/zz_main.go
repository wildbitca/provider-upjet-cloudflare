...
import (
	...
	apisCluster "github.com/wildbitca/provider-upjet-cloudflare/apis/cluster"
	apisNamespaced "github.com/wildbitca/provider-upjet-cloudflare/apis/namespaced"
	"github.com/wildbitca/provider-upjet-cloudflare/config"
	"github.com/wildbitca/provider-upjet-cloudflare/internal/clients"
	controllerCluster "github.com/wildbitca/provider-upjet-cloudflare/internal/controller/cluster"
	controllerNamespaced "github.com/wildbitca/provider-upjet-cloudflare/internal/controller/namespaced"
	"github.com/wildbitca/provider-upjet-cloudflare/internal/features"
	"github.com/wildbitca/provider-upjet-cloudflare/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for UpjetCloudflare").DefaultEnvars()
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-cloudflare-authenticated",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped UpjetCloudflare APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced UpjetCloudflare APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-cloudflare:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_authenticated(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetCloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_authenticated(mgr, namespacedOpts), "Cannot setup namespaced UpjetCloudflare controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_authenticated(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetCloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_authenticated(mgr, namespacedOpts), "Cannot setup namespaced UpjetCloudflare controllers")
	}
