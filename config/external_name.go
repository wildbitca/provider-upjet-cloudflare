...
var ExternalNameConfigs = map[string]config.ExternalName{
	...
	"cloudflare_dns_record": config.NewExternalNameFrom(config.IdentifierFromProvider, config.WithGetIDFn(func(_ config.GetIDFn, _ context.Context, externalName string, parameters map[string]any, _ map[string]any) (string, error) {
		zoneID, _ := parameters["zone_id"].(string)
		if zoneID == "" {
			return "", errors.New("zone_id is required for cloudflare_dns_record external name")
		}
		return fmt.Sprintf("%s/%s", zoneID, externalName), nil
	})),
